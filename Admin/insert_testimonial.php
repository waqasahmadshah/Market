...
if (isset($_FILES['image_url'])) {
    ...
    if (move_uploaded_file($file['tmp_name'], $target_path)) {
        ...
        $short = mysqli_real_escape_string($con, $_POST['Postion']);
        $description = mysqli_real_escape_string($con, $_POST['description']); 
        
        // Insert into database
        $sql = "INSERT INTO testimonial 
                (name,position,description, image_url) 
                VALUES ('$title','$short','$description','$file[name]')";
        
        if (mysqli_query($con, $sql)) {
            header("Location: http://localhost/Marketxsolution/admin/testimonial.php");
            exit();
        } else {
