...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $icon=mysqli_real_escape_string($con,$_POST['position']);
    ...
    if (isset($_FILES['image_url'])) {
        ...
        if ($file['error'] == UPLOAD_ERR_OK) {
            ...
            if (move_uploaded_file($file['tmp_name'], $target_path)) {
                ...
                $result = mysqli_query($con, "SELECT image_url FROM testimonial WHERE id = $id");
    ...
    if (mysqli_query($con, $sql)) {
        ...
        header("Location: http://localhost/Marketxsolution/admin/testimonial.php?status=success");
        exit();
    } else {
