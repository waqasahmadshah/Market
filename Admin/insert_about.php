...
if (isset($_FILES['image_url'])) {
    ...
    if (move_uploaded_file($file['tmp_name'], $target_path)) {
        ...
        $icon=mysqli_real_escape_string($con, $_POST['heading']);  
        
        // Insert into database
        $sql = "INSERT INTO aboutus
                (name,position,heading,description,image_url) 
                VALUES ('$title','$Postion','$icon','$description', '$file[name]')";
        
        if (mysqli_query($con, $sql)) {
            header("Location: http://localhost/Marketxsolution/admin/about.php");
            exit();
        } else {
